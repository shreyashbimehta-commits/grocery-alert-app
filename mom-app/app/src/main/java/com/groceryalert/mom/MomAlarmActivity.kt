...
class MomAlarmActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mom_alarm)
    ...
    private fun dismissAlarm() {
        ...
        if (isBought) {
            MomReminderReceiver.cancelReminder(this)
            if (item.isNotEmpty() && originalSender.isNotEmpty()) {
                MomWebSocketClient.sendBoughtConfirmation(item, originalSender)
            }
        } else {
            if (item.isNotEmpty()) {
                MomReminderReceiver.scheduleReminder(this, item)
            }
        }
